...
export const useSubcategoryStore = create((set, get) => ({
  ...
  fetchSubcategories: async (params = {}) => {
    try {
      ...
      const message =
        err.response?.data?.message || "Failed to fetch subcatagories";
      set({
        loading: false,
        error: message,
      });
  ...
  deleteSubcategory: async (subcategoryId) => {
    try {
      ...
      const newPage =
        pagination.currentPage > 1 && subcategories.length === 1
      ...
      const message =
        err.response?.data?.message || "Subcategory deletion failed";
