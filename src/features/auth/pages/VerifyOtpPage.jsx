...
import { changeEmailApi, verifyEmailChangeApi } from '@/features/user/api/user.api'
...
const OTP_CONFIG= {
	...
	[OTP_TYPES.PASSWORD_RESET]:{
		...
		onSuccess: (navigate, email)=> navigate("/reset-password", {state: {email}})
		},
	...
	<div className='min-h-screen flex items-center justify-center bg-background'>
	  <Card className="w-105 shadow-lg lg:p-8 rounded-none">
		...
		<CardContent className="space-y-5">
			...
			<button onClick={handleResend} disabled={time >0} className={`${time>0 ? "opacity-50 cursor-not-allowed": ""}`}>Resend Code</button>
		</CardContent>
	  </Card>
	</div>
  )
