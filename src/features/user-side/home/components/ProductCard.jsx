...
const ProductCard = ({
  ...
  return (
    ...
    >
      {!isActive && (
        <span className="absolute top-3 left-3 z-10 rounded-full bg-black px-3 py-1 text-[10px] font-semibold tracking-wider text-white uppercase">
          Unavailable
        </span>
      )}
      ...
      <div className="relative overflow-hidden rounded-2xl bg-[#ece7df]">
        ...
        {!existInCart && (
          <button
            type="button"
            className="absolute top-3 right-3 z-20 rounded-full bg-white p-2"
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
